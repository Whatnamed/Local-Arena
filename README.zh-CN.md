...
- 原 Local Arena 的 Bot、Match、Stats 和诊断流程继续保留在这个个人 fork 中
- 四步首次安装向导，自动识别纯净 CS2、旧版兼容安装和上游原版插件
- 事务式备份、安装验证、修复、回滚和恢复纯净 CS2
- 只读查看上游版本和兼容性；个人 fork 通过审查后的 Git 改动和完整本地安装包更新
...
- 面板可执行文件、`addons`、`cfg`、`overrides` 和 `plus-payload-manifest.json` 必须保持在同一个目录
- 不要直接在压缩包里面运行面板
...
**设置 → 在线更新**在这个个人 fork 中仅用于只读查看上游兼容性。个人 fork 的改动应通过审查后的 Git 提交和完整本地安装包应用。
...
默认刀和快捷换刀都通过有界的事件驱动路径应用。掉落和拾取实体遵循原 Local Arena 运行时语义；没有匹配预设时保持当前实体状态

在快捷换刀选择器中启用后，`\\` 会按 Karambit、Butterfly、M9 Bayonet、Bayonet、Skeleton、Falchion 顺序轮换。只有新刀创建并验证成功后才替换旧刀，失败时当前刀仍可恢复

### 人机预设

预设页面用于设置人机瞄准、投掷物行为、快捷换刀列表，以及玩家 CT 与 T 刀具手套入口
...
面板可以检查上游版本和发布说明，但不会把任何上游面板或插件负载安装到这个个人 fork 上

- 检查是只读的，不会下载或激活面板、插件负载
- fork 更新通过审查源码改动、本地构建和 `main` worktree 的完整安装包完成
- 安装、修复和恢复仍遵循保留配置策略，不会覆盖玩家预设
...
网络或上游元数据不可用时，只读参考检查可能暂时失败，但不会改变已安装的个人 fork

保留当前版本，稍后重新检查；相同错误反复出现时导出诊断。不要把只读参考当成可安装更新
