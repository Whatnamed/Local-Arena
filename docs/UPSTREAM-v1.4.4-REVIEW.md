...
- **BotState:** moves from module version `1.8.2` to `1.9.4` and contains substantial fake-defuse, visibility, and flash behavior changes. The independent [Deathmatch elimination guard](https://github.com/ed0ard/CS2-Bot-Improver/commit/6591d1471a623a2a91f5e8c3999c55639316a49c) has been selectively integrated into the existing `OnPlayerDeath` flow. The full v1.4.4 `BotState.cs` also calls `IBotControllerApi.InjectUsercmd`, which the Local Arena ABI 14 interface does not define. A targeted build confirmed `CS1061`; integrating the full source requires the related BotController ABI work.
...
Only the Deathmatch elimination guard from v1.4.4 was selectively integrated. It checks `game_type == 1` and `game_mode == 2` before the existing elimination handling in the current BotState implementation; no full-file replacement or ABI 17 API was introduced. The non-core auto-inspect path that required `InjectUsercmd` was removed so BotState remains compatible with the current ABI 14 contract. Knife switching/locking, reload interruption, and automatic gun re-equipping use existing ABI 14 methods and remain. `build.ps1` builds BotState from this source, and `package.ps1` overlays that output after the upstream payload while verifying the packaged DLL hash against the build output. Other v1.4.4 runtime changes remain deferred. The package/runtime upstream pin stays at v1.4.3 because this small patch is not an overall upgrade. BotAI's changed offsets and patch definitions are Linux-specific, while the current Windows BotHider definitions do not gain a compatible post-update signature from this release.

The native MSVC build simplification is independent of the upstream source review. This consistency follow-up adds the current BotState source to the build list and package overlay without changing other runtime pins.
