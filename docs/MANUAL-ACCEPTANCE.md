...
- [ ] **换刀是替换实体，不是原地改当前刀**：按下后模型、握持姿势、检视动画、音效和刀皮必须同时变。重点检查之前实测出问题的组合（Karambit / Butterfly / M9 / Bayonet / Skeleton / Falchion 之间来回切），不允许出现"手和刀不匹配 / 穿模"。
- [ ] 地面上不应短暂出现旧刀，Bot 或其他玩家不能捡到这次切换产生的实体。
- [ ] 某次切换失败时（控制台会回 `Knife shortcut failed: …`），**手上必须仍有原来那把刀**，且下一次按下重试的是同一目标，不会跳过档位。
- [ ] 当前阵营没给某把刀配皮肤时，切过去应当是 **Vanilla 原皮**，而不是拒绝切换，也不能借用另一阵营的皮肤。
- [ ] 每把刀切换后恢复该刀自己的皮肤预设。
- [ ] 快捷功能关闭时不修改 `\` 或其他按键。
...
- **购买菜单里显示的仍是你真实库存的皮肤外观**，实际买出来的 weapon entity 才使用 Local Cosmetics 预设。本项目的写入是服务端在武器实体创建之后写经济属性，购买菜单由客户端真实库存 UI 渲染；要让菜单预览也显示自定义皮肤必须改原生 HUD / 加游戏内 Overlay，越过 `docs/PRODUCT-SCOPE.md` 的边界，明确不做。
- 预设导出是**整套 Cosmetics Preset**：一个文件同时包含 CT 与 T 的刀、手套、枪皮、音乐盒、开关与快捷刀轮换；导入是整体替换而不是合并，替换前会先备份当前配置。不要按"只导出当前阵营"理解。
...
- [ ] **游戏退出后 Panel 窗口能恢复**：最小化、被 CS2 全屏压到后面、或关闭游戏后，再次双击 Panel 可执行文件（触发单实例恢复）必须把窗口带回前台。
- [ ] 上一条失败时，检查 `.csbip/logs/panel/panel-current.jsonl` 里的 `panel.restore_window` 行：它会记录 `before visible=… minimized=…` 以及 `unminimize / show / focus` 三个动作各自的结果。
- [ ] 已知未定论情形：如果日志显示三个动作都 `=ok`、窗口状态是 visible 且未最小化，但画面仍然黑着 / 不重绘，那就不属于窗口状态问题，而是无边框 + 透明窗口的合成表面问题（下一阶段再单独验证 `transparent: false`）。当前阶段不加托盘、不改透明度。
