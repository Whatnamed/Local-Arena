...
## 2026-09-24 Human crash hotfix 首轮顺序（尚未实机验证）

先在 Enhanced Bots 中按下面顺序单次验证。后面的五圈压力项只在单次路径稳定后进行。

1. 进入游戏后站着不操作 10 秒；确认没有自动切刀音效、HUD 刀图标闪跳，且按 1/2/3 正常。
2. 检查出生默认刀和 CT/T 手套；记录刀型、皮肤、第一人称手套及是否有裸手或重叠。
3. 只按一次 \，等待 2 秒；检查目标刀型、皮肤、动画、HUD、音效，确认没有持续切槽。
4. 再按第二次 \，重复单次检查；之后才走完整一圈。
5. 单次稳定后再做五圈、死亡重生和换队。若客户端再崩溃，保留当次插件日志和 crash dump。
