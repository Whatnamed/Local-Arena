...
import { localizedSkinName, matchesCosmeticSearch, skinNameAliases } from "../data/skinLocalization";
...
export default function WeaponPresetModal({ weapon, team, csgoPath, config, onSaved, onError, onClose }: Props) {
  ...
  const visible = useMemo(() => {
    return allSkins.filter((skin) => matchesCosmeticSearch(query, [
      localizedSkinName(appConfig?.language, skin.weapon_defindex, skin.paint, skin.name),
      localizedSkinName("english", skin.weapon_defindex, skin.paint),
      ...skinNameAliases(skin.weapon_defindex, skin.paint), skin.paint, skin.weapon_defindex,
    ]));
